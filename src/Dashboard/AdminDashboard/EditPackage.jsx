...
const EditPackage = () => {
  ...
  useEffect(() => {
    const fetchPackage = async () => {
      try {
        setIsLoading(true);
        const res = await get(`/get-package/${id}`);
        ...
        if (res?.success && res.data) {
          ...
          setCategory(data.category || "City");
          setMainImage(data.images?.[0] || "");

          // Ensure at least 3 gallery fields
          const loadedGallery = [
            ...(data.images?.slice(1) || []),
            "",
            "",
          ...
          console.error("Package not found or invalid response");
        }
      } catch (err) {
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const res = await put(`/update_package/${id}`, updatedPackage);
    ...
    if (res?.success) {
      ...
      navigate("/admin-dashboard/manage-packages");
    } else {
