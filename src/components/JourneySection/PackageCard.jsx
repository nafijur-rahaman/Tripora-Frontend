...
import { FiHeart, FiStar, FiClock } from 'react-icons/fi'; // npm install react-icons

const PackageCard = ({ pkg, variants }) => {
    return (
        <motion.div
            variants={variants}
            whileHover={{ scale: 1.03 }} // Card scales up on hover
            transition={{ type: "spring", stiffness: 300, damping: 10 }}
            className="relative h-[450px] rounded-2xl overflow-hidden shadow-2xl group cursor-pointer"
        >
            {/* 1. Background Image with Hover Effect */}
            <img
                src={pkg.image}
                alt={pkg.title}
                className="absolute inset-0 w-full h-full object-cover 
                           transition-transform duration-500 ease-in-out
                           group-hover:scale-110" // Image zooms in on hover
            />

            {/* 2. Gradient Overlay for Text Readability */}
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent"></div>

            {/* 3. UI Elements (Wishlist, Duration) */}
